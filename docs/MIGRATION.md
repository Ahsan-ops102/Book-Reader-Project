...
- Missing PDF covers are extracted automatically on library open, once per visit, without replacing saved covers. The **Fill missing PDF covers** action can retry failed extractions. Use published lookup per book when the publisher's cover is missing from the PDF.
...
Service-worker updates activate after the complete new app shell has been cached, without reloading existing tabs. Refresh to load the new interface; save/export unsynced drafts before refreshing. Previous hashed bundles remain cached while existing windows are open, so their readers/editors can continue safely. The worker validates the cached HTML against its asset manifest, so deployments should publish HTML, assets and the worker together; keep older assets available during rollout.
