...
import { fixTextWithAI, transformTextWithAI } from "../api.js";
import "./Writer.css";

/* ── SVG icon helpers (crisp, no emoji dependency) ────── */
const icons = {
  ...
  return (
    <div className="find-replace-panel">
      <div className="find-replace-row">
        ...
        <button className="find-action-btn" onClick={handleReplace} disabled={!findText}>
          Replace
        </button>
        <button className="find-action-btn accent" onClick={handleReplaceAll} disabled={!findText}>
          Replace All
        </button>
...
function AIToolsPanel({ editor, onClose }) {
  ...
  async function runOp(opId) {
    ...
    try {
      ...
      setResult({ opId, error: err.message || "Failed" });
    } finally {
      setProcessing(null);
    }
  }
  ...
  return (
    <div className="ai-tools-panel">
      <div className="ai-tools-header">
        <span className="ai-tools-title">{icons.wand} AI Writing Tools</span>
        <button className="find-close-btn" onClick={onClose}>{icons.clear}</button>
      </div>
      ...
      <div className="ai-ops-grid">
        {AI_OPERATIONS.map((op) => (
          <button
            key={op.id}
            className={`ai-op-card ${processing === op.id ? "processing" : ""}`}
            onClick={() => runOp(op.id)}
            disabled={!!processing}
          >
            <span className="ai-op-icon">{op.icon}</span>
            <span className="ai-op-label">{op.label}</span>
            <span className="ai-op-desc">{op.desc}</span>
      ...
      {result && (
        <div className={`ai-result-box ${result.error ? "error" : ""}`}>
          {result.error ? (
            <p className="ai-result-error">⚠️ {result.error}</p>
          ) : (
            <>
              <p className="ai-result-label">
                ✨ {AI_OPERATIONS.find((o) => o.id === result.opId)?.label} Result:
              </p>
              <div className="ai-result-preview">{result.text}</div>
              <div className="ai-result-actions">
                <button className="ai-result-apply" onClick={applyResult}>
                  Apply to Document
                </button>
                <button className="ai-result-discard" onClick={() => setResult(null)}>
                  Discard
                </button>
              </div>
            </>
...
function WriterToolbar({
  editor, onFix, fixing, onToggleFindReplace, showFindReplace,
  onToggleAITools, showAITools, onToggleFullscreen, isFullscreen,
  onPrint, showHighlightPicker, setShowHighlightPicker,
  showTextColorPicker, setShowTextColorPicker,
}) {
  if (!editor) return null;

  return (
    <div className="writer-toolbar-container">
      <div className="writer-toolbar">
        ...
        <div className="toolbar-dropdown-wrap">
          ...
          {showHighlightPicker && (
            <div className="toolbar-color-picker">
              ...
              <button
                className="color-swatch remove-color"
        ...
        <div className="toolbar-dropdown-wrap">
          <button
            ...
            title="Text Color"
          >
            <span style={{ borderBottom: "3px solid var(--accent)", paddingBottom: "1px", fontWeight: 700, fontSize: "13px" }}>A</span>
          </button>
          {showTextColorPicker && (
            <div className="toolbar-color-picker">
              {TEXT_COLORS.map((c) => (
                <button
                  ...
                  style={{ color: c.value === "inherit" ? "var(--text)" : c.value }}
                  title={c.name}
                  onClick={() => {
        ...
        {/* Alignment */}
        <button className={`toolbar-btn ${editor.isActive({ textAlign: "left" }) ? "active" : ""}`} onClick={() => editor.chain().focus().setTextAlign("left").run()} title="Align Left">{icons.alignLeft}</button>
        <button className={`toolbar-btn ${editor.isActive({ textAlign: "center" }) ? "active" : ""}`} onClick={() => editor.chain().focus().setTextAlign("center").run()} title="Align Center">{icons.alignCenter}</button>
        <button className={`toolbar-btn ${editor.isActive({ textAlign: "right" }) ? "active" : ""}`} onClick={() => editor.chain().focus().setTextAlign("right").run()} title="Align Right">{icons.alignRight}</button>
        ...
        <button className="toolbar-btn" onClick={() => editor.chain().focus().setHorizontalRule().run()} title="Horizontal Rule">{icons.hr}</button>

        <div className="toolbar-separator" />

        {/* Undo / Redo */}
        <button className="toolbar-btn" onClick={() => editor.chain().focus().undo().run()} disabled={!editor.can().undo()} title="Undo (⌘Z)">{icons.undo}</button>
        <button className="toolbar-btn" onClick={() => editor.chain().focus().redo().run()} disabled={!editor.can().redo()} title="Redo (⌘⇧Z)">{icons.redo}</button>

        <div className="toolbar-separator" />

        {/* Utilities */}
        <button className={`toolbar-btn ${showFindReplace ? "active" : ""}`} onClick={onToggleFindReplace} title="Find & Replace (⌘F)">{icons.search}</button>
        ...
        {/* AI Tools Toggle */}
        <button
          className={`toolbar-btn toolbar-btn-ai-toggle ${showAITools ? "active" : ""}`}
        ...
        {/* AI Fix */}
        <button
          className={`toolbar-btn toolbar-btn-fix ${fixing ? "fixing" : ""}`}
          onClick={onFix}
          disabled={fixing}
          title="Fix all grammar & spelling errors with AI"
        >
          {fixing ? (
            <><span className="fix-spinner" /> Fixing…</>
          ) : (
            <>{icons.sparkles} Fix All Errors</>
          )}
        </button>
      </div>
    </div>
...
export default function Writer() {
  ...
  useEffect(() => {
    ...
    const saveTimer = setInterval(() => {
      const html = editor.getHTML();
      if (html && html !== "<p></p>") {
        localStorage.setItem("writer_autosave", JSON.stringify({ title: docTitle, html, ts: Date.now() }));
        setLastSaved(new Date());
      }
    }, 15000); // Every 15s
    return () => clearInterval(saveTimer);
  }, [editor, isEditing, docTitle]);

  // Keyboard shortcut: Cmd+F for find/replace
  useEffect(() => {
  ...
  function handleDrop(e) { e.preventDefault(); e.stopPropagation(); setIsDragging(false); dragCounterRef.current = 0; const file = e.dataTransfer.files?.[0]; if (file) loadDocx(file); }
  ...
  // Resume auto-saved
  function resumeAutoSaved() {
    try {
      const saved = JSON.parse(localStorage.getItem("writer_autosave"));
      if (saved?.html) {
        setDocTitle(saved.title || "Untitled Document");
  ...
  const hasAutoSave = useMemo(() => {
    try {
      const saved = JSON.parse(localStorage.getItem("writer_autosave"));
      return saved?.html && saved.html !== "<p></p>";
    } catch { return false; }
  }, []);
  ...
  async function handleFix() {
    ...
    try {
      ...
      setBanner({ type: "error", msg: `Failed to fix text: ${err.message || "Unknown error"}` });
  ...
  function handlePrint() {
    ...
    printWindow.document.write(`
      ...
      <style>
        ...
        h1 { font-size: 28px; margin-bottom: 8px; } h2 { font-size: 22px; } h3 { font-size: 18px; }
      ...
      <body><h1>${docTitle}</h1>${html}</body></html>
    `);
      ...
      <div className="writer writer-landing" onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} onDragOver={handleDragOver} onDrop={handleDrop}>
        <header className="writer-header">
          <div className="writer-brand">
            ...
            <p className="writer-subtitle">Your vibrant AI-powered document editor — load, write, perfect.</p>
          </div>
        ...
        <div className="writer-landing-grid">
          <div className={`writer-dropzone ${isDragging ? "dragging" : ""}`} onClick={() => fileInputRef.current?.click()}>
            ...
            <p>Drag & drop a <strong>.docx</strong> file or click to browse</p>
          </div>

          <div className="writer-dropzone blank-zone" onClick={startBlank}>
            <div className="dropzone-glow glow-alt" />
            <span className="drop-icon">✍️</span>
            <h2>Start Fresh</h2>
            <p>Begin with a blank canvas and write freely</p>
          </div>

          {hasAutoSave && (
            <div className="writer-dropzone resume-zone" onClick={resumeAutoSaved}>
              <div className="dropzone-glow glow-resume" />
              <span className="drop-icon">🔄</span>
              <h2>Resume Draft</h2>
              <p>Continue where you left off with your auto-saved document</p>
            </div>
          )}
        </div>

        <div className="writer-features-bar">
          <span className="feature-chip">✨ AI Error Fix</span>
          <span className="feature-chip">🔄 Paraphrase</span>
          <span className="feature-chip">🎩 Tone Control</span>
          <span className="feature-chip">🔍 Find & Replace</span>
          <span className="feature-chip">🖨️ Print</span>
          <span className="feature-chip">💾 Auto-Save</span>
          <span className="feature-chip">📥 Export .docx</span>
        </div>
  ...
  return (
    <div className={`writer ${isFullscreen ? "writer-fullscreen" : ""}`}>
      ...
      {banner && (
        <div className={`writer-fix-banner ${banner.type}`}>
          <span>{banner.msg}</span>
          <button className="banner-close" onClick={() => setBanner(null)}>{icons.clear}</button>
        </div>
      )}

      <WriterToolbar
        editor={editor} onFix={handleFix} fixing={fixing}
        onToggleFindReplace={() => setShowFindReplace((v) => !v)} showFindReplace={showFindReplace}
        onToggleAITools={() => setShowAITools((v) => !v)} showAITools={showAITools}
        ...
        showTextColorPicker={showTextColorPicker} setShowTextColorPicker={setShowTextColorPicker}
      />
      ...
      <div className="writer-stats-bar">
        ...
        <span className="stat-item"><span className="stat-label">Sentences</span><span className="stat-value">{stats.sentences.toLocaleString()}</span></span>
        <span className="stat-item"><span className="stat-label">Paragraphs</span><span className="stat-value">{stats.paragraphs.toLocaleString()}</span></span>
        ...
        {lastSaved && (
          <span className="stat-item auto-save-indicator">
            <span className="save-dot" />
            <span className="stat-label">Saved {lastSaved.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
          </span>
        )}
      </div>
