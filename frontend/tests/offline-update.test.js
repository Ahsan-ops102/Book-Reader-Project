...
function worker({wrongHtml=false,failedDownload=false}={}){
 ...
 vm.runInNewContext(source,{self,caches,URL,Request:class{constructor(url){this.url=url}},fetch:async()=>({ok:true,clone(){return this},text:async()=>`<script src="/assets/${wrongHtml?'wrong':'new'}.js"></script>`})});
 return {handlers,events,stores};
}
